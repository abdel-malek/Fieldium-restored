...
public class UserController extends ParentController {
    ...
    public void change_password (int user_id,String oldPass,String newPass , SuccessCallback<User> successCallback) {
        String url= new URLBuilder(APIModel.users,"change_password").getURL();
        TradinosRequest request = new TradinosRequest(getmContext(),url, RequestMethod.Post,new UserParser(), successCallback,getmFaildCallback()) ;
        request.addParameter("user_id", String.valueOf(user_id));
        request.addParameter("password", newPass);
        request.addParameter("old_password", oldPass);
    ...
    public void updateToken (int user_id,String token , SuccessCallback<String> successCallback) {
