...
import com.bluebrand.fieldiumadmin.Parser.DeclinedReservationsReportParser;
...
public class ReportsController extends ParentController {
    ...
    public void declinedReservationsReport (int field_id,String from,String to ,SuccessCallback<DeclinedsReservationsReport> successCallback) {
        String url= new URLBuilder(APIModel.reports,"declined_reservations_report").getURL();
        TradinosRequest request = new TradinosRequest(getmContext(),url, RequestMethod.Get,new DeclinedReservationsReportParser(), successCallback,getmFaildCallback()) ;
        request.addParameter("field_id",String.valueOf(field_id));
        request.addParameter("from_date",from);
        request.addParameter("to_date",to);
