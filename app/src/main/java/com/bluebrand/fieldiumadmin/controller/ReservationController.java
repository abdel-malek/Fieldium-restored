...
import com.bluebrand.fieldiumadmin.Parser.RefusedMsgParser;
...
public class ReservationController extends ParentController {
    ...
    public void getReservations (SuccessCallback<List<Reservation>> successCallback) {
    ...
    public void cancelReservation (int booking_id,int reason_id,String note,SuccessCallback<String> successCallback) {
        String url= new URLBuilder(APIModel.bookings,"cancel").getURL();
        TradinosRequest request = new TradinosRequest(getmContext(),url, RequestMethod.Get,new StringParser(), successCallback,getmFaildCallback()) ;
        request.addParameter("booking_id",String.valueOf(booking_id));
        request.addParameter("reason_id",String.valueOf(reason_id));
    ...
    public void creatReservation (Map<String,String> Details, SuccessCallback<Reservation> successCallback) {
        ...
        request.addParameter("game_type",Details.get("game_type"));
        authenticationRequired(request);
        request.Call();
    }

    public void getRefusedMsgs (SuccessCallback<List<RefusedMsg>> successCallback) {
        String url= new URLBuilder(APIModel.bookings,"get_cancellation_reasons").getURL();
        TradinosRequest request = new TradinosRequest(getmContext(),url, RequestMethod.Get,new RefusedMsgParser(), successCallback,getmFaildCallback()) ;
        authenticationRequired(request);
        request.Call();
    }
}
