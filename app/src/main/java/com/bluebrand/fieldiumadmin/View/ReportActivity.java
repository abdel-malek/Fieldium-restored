    ...
    void getData() {
      ...
      /*  ReportsController.getInstance(getmController()).reservationsReport(((MyApplication) getApplication()).getMyCompanyInfo().getID(), fromDateString, toDateString, new SuccessCallback<ReservationsReport>() {
                ...
                public void OnSuccess(ReservationsReport result) {
                    HideProgressDialog();
                    reportsAdapter=new ReportsAdapter(ReportActivity.this,R.layout.report_item,result.getReservationReportList(),report_listView);
                    report_listView.setAdapter(reportsAdapter);
                    total_reservations.setText(String.valueOf(ReservationsReport.getAll_bookings_number()));
                    total.setText(String.valueOf(ReservationsReport.getAll_total())+" AED");
                }
            });*/
    }
