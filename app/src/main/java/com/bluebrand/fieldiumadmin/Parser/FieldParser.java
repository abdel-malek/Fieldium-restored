...
public Field Parse(String jsonText) throws JSONException {
        ...
        public Field Parse(JSONObject jsonObject) throws JSONException{
            ...
            field.setAutoConfirm(jsonObject.getInt("auto_confirm")==1?true:false);
            ...
            if(jsonObject.getJSONArray("children").length()!=0)
                field.setChildsFieldsIds(getChildren(jsonObject.getJSONArray("children")));

            return field;
        }

        private ArrayList<Integer> getChildren(JSONArray jsonArray){
            ArrayList<Integer> ids=new ArrayList<>();
            for (int i=0; i<jsonArray.length();i++) {
                try {
                    ids.add(jsonArray.getJSONObject(i).getInt("field_id"));
