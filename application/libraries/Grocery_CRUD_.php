	...
	{	
		...
		else
		{
            if($has_extra_fields){
                ...
                if(!empty($selected_values))
                    foreach($selected_values as $id => $name){
                        $input .= "<li><a href='#actors-relation-$id'>$name</a><span class='ui-icon ui-icon-close' data-value='$id' data-field='{$field_info_type->name}' role='presentation'>Remove Tab</span></li>";
                    }
                ...
                if(!empty($selected_values))
                    foreach($selected_values as $id => $name){
                        ...
                        $input .= "<div id='actors-relation-$id'><p></p>";
...
class grocery_CRUD extends grocery_CRUD_States
{
	...
	const	JQUERY_UI_JS 	= "jquery-ui-1.9.2.custom.min.js";
