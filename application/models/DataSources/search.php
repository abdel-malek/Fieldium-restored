...
class search extends CI_Model {
    ...
    public function search($name, $game, $area, $lang = "en") {
        ...
        if ($name != "") {
            $this->db->where("((field.en_name like '%$name%') OR (company.en_name like '%$name%') OR (area.en_name like '%$name%') OR (company.en_address like '%$name%'))");
        } else {
            if ($area != 0)
    ...
    public function search_with_timing($name, $game, $area, $timing, $start, $duration, $date, $lang = "en") {
        $hour = date('H');
        if (date('i') > "00")
            $hour++;
        $current = $hour . ":00:00";
