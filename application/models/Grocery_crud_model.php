    	...
    	{    
    		foreach($post_data as $primary_key_value)
	    	{
				...
				if($count == 0)
				{
					...
					$this->db->insert($field_info->relation_table, array_merge($where_array, @$extra_data[$primary_key_value]));
					
				}
                elseif($count >= 1 && !empty($field_info->priority_field_relation_table))
				{
					$this->db->update( $field_info->relation_table, array_merge(array($field_info->priority_field_relation_table => $counter), @$extra_data[$primary_key_value]) , $where_array);
				}
                elseif($count >= 1) {
